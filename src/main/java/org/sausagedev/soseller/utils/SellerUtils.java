...
public class SellerUtils {
    ...
    public SellerUtils(SoSeller main) {
        this.main = main;
    }
    ...
    public void add(UUID uuid, Connection connection) throws SQLException {
        ...
        PreparedStatement ps3 = connection.prepareStatement("INSERT INTO database(uuid, nick, items, boost) VALUES (?, ?, ?, ?)");
    ...
    public boolean isBoughtAutoSell(UUID uuid) {
        if (main.getConfig().getInt("auto-sell.cost", 100) <= 0) return true;
        try {
            Connection connection = main.getConnection();
